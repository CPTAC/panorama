...
# {
#     current_number <- 2
#     all_data_files <- list.files("../test_data")[2 : 151]
#     current_data_file <- all_data_files[current_number - 1]
#     QC_set <- read.csv(paste("../test_data/",
#     current_data_file,
#     sep = ""))
#
...
#     dataset_path <- "CPTAC_TEST/Experiment_3_TEST"
#     input_protein_name <- "PSMD1.IPI00299608"
#     input_peptide_sequence <- "AAVESLGFILFR"
...
QC_set[,'concentration'] <- as.character(QC_set[,'concentration'])
QC_set[,'sample_group'] <- as.numeric(QC_set[,'sample_group'])
...
cell_lines <- sort(unique(QC_set[ , 'concentration']))

# get a list of all unique spike levels (0,5,10) associated with current peptide
spike_levels <- sort(unique(QC_set[ , 'sample_group']))
  ...
  for (current_cell_line in cell_lines) {
    ...
    for (current_spike_level in spike_levels) {
      ...
      for (current_rep in replicates) {
        ...
        current_set <- QC_set[QC_set$fragment_ion==current_ion & QC_set$concentration==current_cell_line & QC_set$sample_group==current_spike_level & QC_set$replicate==current_rep, ]
