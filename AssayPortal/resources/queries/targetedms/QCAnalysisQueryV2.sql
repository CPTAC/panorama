       ...
       CASE
           WHEN tci.TransitionId.NeutralLossMass > 0
               ...
               (tci.TransitionId.FragmentType || tci.TransitionId.FragmentOrdinal || '-' ||
                ceiling(tci.TransitionId.NeutralLossMass))
           ELSE
               (tci.TransitionId.FragmentType || tci.TransitionId.FragmentOrdinal)
           END
                                                                            AS FragmentIon,
...
--rannot1.Value AS Replicate,
       (SELECT rannot2.Value
        FROM replicateannotation rannot2
        WHERE tci.SampleFileId.replicateId = rannot2.replicateid
          AND rannot2.Name = 'Day')                                         AS Day,
--rannot2.Value AS Concentration
       (SELECT rannot3.Value
        FROM replicateannotation rannot3
        WHERE tci.SampleFileId.replicateId = rannot3.replicateid
          AND rannot3.Name = 'Exp2 SampleGroup')                            AS Exp2SampleGroup,
